...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out) {
    ...
    for (QList<QPair<QString, QString>>::iterator i = items.begin();
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::BCA, i->second,
                                         &rv.amount)) {
                    return false;
                }
            }
...
QString formatBitcoinURI(const SendCoinsRecipient &info) {
    ...
    if (info.amount) {
        ret +=
            QString("?amount=%1")
                .arg(BitcoinUnits::format(BitcoinUnits::BCA, info.amount, false,
        ...
        if (mask & check) {
            switch (check) {
                ...
                case NODE_XTHIN:
                    ...
                    break;
		case NODE_BITCOIN_ABCANOSA:
		    strList.append("ABCANOSA");
		    break;
                default:
